...
- **`networkRecoveryInterval`**: no SDK default (omitted stays unset and fails startup validation). Unit remains **milliseconds** (Spring AMQP) — not seconds (unlike .NET). Sample updated from `3000` to `5000` (5 seconds). Must be **positive**; `0`/negatives/omitted are rejected (`0` would busy-loop Spring AMQP recovery) (TRGN-4200).
